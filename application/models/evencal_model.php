...
class Evencal_model extends CI_Model {
	...
	function getEvent($year, $month, $day){
		...
		$query = $this->db->select('idevent as id, event_time as time, event_date, event, lugar, imag, tip, detalle, region, temperatura, estacion')->order_by('event_time')->get_where('event_detail', array('event_date' => $year));
		if($query->num_rows() > 0){
